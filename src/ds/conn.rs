...
use crate::proto::tcp::inbound::*;
use crate::proto::tcp::outbound::*;
...
pub(crate) async fn udp_conn(
    ...
    println!("Trying to yeetify a socket boi");
    let mut udp_tx = UdpSocket::bind("0.0.0.0:0").await?;
    println!("Trying to connect");
    udp_tx.connect(&format!("{}:1110", target_ip)).await?;
//    let mut udp_tx = UdpFramed::new(udp_tx, DsUdpCodec);
...
//    let mut stream = select(udp_rx.map(Either::Left), rx.map(Either::Right));
    ...
    loop {
        let mut item = Some(Either::<crate::Result<(UdpResponsePacket, SocketAddr)>, Never>::Left(udp_rx.next().await.unwrap()));
        println!("Item yielded in UDP loop");
            ...
            match item {
                Either::Left(packet) => {
                    if let Ok(packet) = packet {
                        ...
                        if packet.seqnum != _state.seqnum() {
                            continue;
                        }
                        ...
                        if packet.need_date {
                            ...
                            _state.queue_udp(UdpTag::DateTime(tag));

                        }

                        if !tcp_connected {
                            tokio::spawn(tcp_conn(state.clone(), target_ip.clone()));
                            tcp_connected = true;
                        }

                        _state.set_trace(packet.trace);
                        _state.set_battery_voltage(packet.battery);
                        _state.increment_seqnum();
                    }
                }
...
pub(crate) async fn tcp_conn(state: Arc<Mutex<State>>, target_ip: String) -> Result<()> {
    let mut conn = TcpStream::connect(&format!("{}:1740", target_ip)).await?;
    let mut codec = DsTcpCodec.framed(conn);

    loop {
        let packet = codec.next().await;
...
//    let udp_tx = UdpFramed::new(udp_tx, DsUdpCodec);
//
